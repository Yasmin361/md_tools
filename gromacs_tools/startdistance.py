...
def getpaths(cwd, mode):
    ...
    subgroups = ["fingerloop_TMD.ndx"]  # supply appropriate index file names
    #subgroups = os.listdir(ndx_path)
    return trajectories, trjs_path, subgroups, ndx_path


# calculate distance of given groups over trajectories
def calculatedist(cwd, mode):
    ...
    if mode == "local":
        ...
        for traj in trajectories:
            trajn = traj.split(".")[0]
            trj_path = os.path.join(trjs_path, traj)
            outpath = os.path.join(cwd, "results", "distance", trajn)  # outpath cluster
